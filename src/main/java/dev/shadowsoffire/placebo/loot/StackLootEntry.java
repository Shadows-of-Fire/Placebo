...
public class StackLootEntry extends LootPoolSingletonContainer {
    ...
    public static final MapCodec<StackLootEntry> CODEC = RecordCodecBuilder.mapCodec(inst -> inst
        .group(
            ItemStackTemplate.CODEC.fieldOf("stack").forGetter(e -> e.stack),
            Codec.intRange(0, 99).fieldOf("min").forGetter(e -> e.min),
            Codec.intRange(0, 99).fieldOf("max").forGetter(e -> e.max))
    ...
    public StackLootEntry(ItemStackTemplate stack, int min, int max, int weight, int quality, List<LootItemCondition> conditions, List<LootItemFunction> functions) {
    ...
    public StackLootEntry(ItemStackTemplate stack, int min, int max, int weight, int quality) {
    ...
    public StackLootEntry(ItemLike item, int min, int max, int weight, int quality) {
        this(new ItemStackTemplate(item.asItem().builtInRegistryHolder()), min, max, weight, quality);
    }

    public StackLootEntry(ItemStackTemplate stack) {
        this(stack, 1, 1, 1, 0);
    }

    @Override
    protected void createItemStack(Consumer<ItemStack> list, LootContext ctx) {
        ItemStack s = this.stack.create();
