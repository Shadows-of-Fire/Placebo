...
public class PlaceboClient {
	...
	/**
	 * Used to register a custom color that extends TextColor.
	 * This is not required for any static color values, because they can be represented as a hex int.
	 * This is, however, required for ANY subclass of TextColor so it can be deserialized on the client.
	 */
	public static <T extends TextColor> void registerCustomColor(String id, T color) {
		TextColor.NAMED_COLORS.put(id, color);
	}
	...
	public static class RainbowColor extends TextColor {
		...
		@Override
		public int getValue() {
			return colors[(int) ((ticks + Minecraft.getInstance().getDeltaFrameTime()) / 0.5F % 255)];
		}
