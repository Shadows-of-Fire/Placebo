...
```java
output = min + (int) (steps * (level + 0.5F / steps)) * step;
```
...
```js
{
    ...
    "max": float,     // [Mandatory] || Maximum value of the output range.
    "step": float     // [Optional]  || The value that an individual step will increase the output by. Must evenly divide the difference between min and max, or an error will be raised. Default value = 0.01.
}
...
The last form is deprecated, and will be removed in a future version. Existing files should be converted to the bounds form.

Some implementations may only accept one of the above. If unspecified, it can be assumed to be all three.
